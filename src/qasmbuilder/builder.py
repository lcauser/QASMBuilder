...
class Builder:
    ...
    def initialise(self):
        """
        Initialise the QASM script.
        """

        qasm = f"OPENQASM {self.version}; \n"
        qasm += f"include \"stdgates.inc\";\n\n"
        qasm += f"qubit[{self.num_qubits}] q; \n"
        qasm += f"bit[{self.num_cbits}] c; \n\n"
        return qasm 
    ...
    def cnot(self, qasm: str, control: int, target: int):
        ...
        """
        return self._two_qubit_gate(qasm, control, target, "cx")
